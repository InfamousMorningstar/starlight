...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    hasFadedRef.current = true; // Skip fade-in for instant display
    ...
    // Initial render to ensure visibility on mount
    setTimeout(() => {
    ...
    const io = new IntersectionObserver(
      entries => {
        ...
        // Force a render when coming into view
        if (isIntersecting && pausedRef.current === false) {
          renderer.render(scene, camera);
        }
      },
    ...
    const onVis = () => {
      const wasHidden = pausedRef.current;
      pausedRef.current = document.hidden;
      // Force a render when becoming visible
    ...
    mount.addEventListener('pointermove', onMove as any, { passive: true });
    mount.addEventListener('pointerdown', onMove as any, { passive: true });
    mount.addEventListener('pointerenter', onMove as any, { passive: true });
    mount.addEventListener('pointerleave', onLeave as any, { passive: true });
    ...
    return () => {
      ...
      mount.removeEventListener('pointermove', onMove as any);
      mount.removeEventListener('pointerdown', onMove as any);
      mount.removeEventListener('pointerenter', onMove as any);
      mount.removeEventListener('pointerleave', onLeave as any);
